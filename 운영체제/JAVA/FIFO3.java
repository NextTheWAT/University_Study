...
public class FIFO3 {//클래스 생성
      ...
      public static void main(String[] args) {//어디서든 접근 가능 public 이며 이미 메모리 할당 된 main메소드 선언
        ...
        System.out.print("입력 하고자 하는 값 입력 : ");//출력문
        mem=c.nextInt();//입력 받은 값을 mem변수에 저장
         ...
         if((cnt==3)&&(cnt2==0)) {//cnt=3 검사 시 3번 증가하였으므로 0,1,2 모든 인덱스가 공백이 아닌 즉 포화상태, 그리고 cnt2=0 중복이 없다는 뜻으로 즉 삽입하려는 값과 삽입을 하려는 배열간에 중복이 없고 포화상태일때 현재 존재하는 값들을 앞으로 옮기고 맨 뒤에 값을 삽입
            int max=0;
            for(int q=0; q<3; q++) {
             if(count[q]>max) {
                max=count[q];
