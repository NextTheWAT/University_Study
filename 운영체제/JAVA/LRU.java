...
public class LRU { // FIFO2라는 객체를 생성
	...
	public static void main(String[] args) { // 접근 한정자를 public으로 설정(어디서든지 사용가능)
		...
		System.out.println("IT융합학부 1971070 2학년 이재은 LRU" + "\n"); // 학과, 학번, 학년, 이름, 과제명 출력문
		...
		for (int i = 0; i < 10; i++) { // 0 ~ 4까지 1++ 조건식이 트루일때만 실행 그리고 문장을 갔다가 증감식을가고 다시 조건식
			...
			} else if (arr[0] == r[i] && ct2 != 0) { // arr[0]값이 현재 입력값과 같고 중복이 있다면 실행
				temp2 = arr[0]; // arr[0]값과 arr[1]값 자리를 바꿔주고 arr[2] 값에 현재 들어오는 값 입력
				arr[0] = arr[1];
				arr[1] = arr[2];
				arr[2] = r[i];
			} else if (arr[1] == r[i] && ct2 != 0) {// arr[1]값이 현재 입력값과 같고 중복이 있다면 실행
				temp2 = arr[0]; // arr[0]을 temp2에 저장한 뒤 arr[1]값에 arr[2]값을 저장해주고 arr[2]값에 현재 들어오는 값 입력
				arr[1] = arr[2];
				arr[2] = r[i];
			} else if (arr[2] == r[i] && ct2 != 0) {// arr[2]값이 현재 입력값과 같고 중복이 있다면 실행
				temp2 = arr[0]; // arr[0]값을 temp2에 저장한뒤 arr[0]값에 arr[1]값을 넣어주고 arr[1]값에 arr[2]값을 넣어준뒤
				arr[0] = arr[1]; // arr[2]값에 현재 들어오는 값 입력
				arr[1] = arr[2];
				arr[2] = r[i];
			}
