...
public class Test3 { // Bubble_Sort라는 클래스 객체를 생성
	public static void main(String[] args) {// 접근 한정자를 public으로 설정(어디서든지 사용가능)
		...
		for (int j = 0; j < 2; j++) { // 0~4까지 1씩 증가//조건식이 트루일때만 실행 그리고 문장을 갔다가 증감식을가고 다시 조건식
			...
			for (int d = 4; d >= 0; d--) { // 0~3까지 1씩 증가 //조건식이 트루일때만 실행 그리고 문장을 갔다가 증감식을가고 다시 조건식
				if (arr[d] < arr[d - 1]) { // arr[i]가 a[i+1]보다 클때 실행
					temp = arr[d]; // temp, arr[i] 을 템프로 교환
					arr[d] = arr[d - 1]; // arr[i] 와 arr[i+1]를 교환
					arr[d - 1] = temp; // arr[i+1]을 temp와 교환
				}
				System.out.print(j + 1 + " - " + d + 1 + "번째 정렬 : ");
				for (int c = 0; c < 5; c++) {
					System.out.print(arr[c] + " ");
				}
				System.out.println("");
