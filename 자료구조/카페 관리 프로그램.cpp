...
int main()  //¸ÞÀÎÈ­¸é
{
   ...
   while(sel!=14)
   {
      ...
      printf("¦­  11. ¸Þ´º ÀüÃ¼ º¸±â        ¦­\n");
      printf("¦­  12. °í°´ ÀüÃ¼ º¸±â        ¦­\n");
      printf("¦­  13. ¹øÈ£Ç¥ ÀüÃ¼ º¸±â      ¦­\n");
...
int search_menu()
{
   ...
   {
      if(strcmp(drink, menu_info[i].drink)==0){ //strcmpÇÔ¼ö »ç¿ë -> ÀÔ·ÂÇÑ drink°ú ±¸Á¶Ã¼ ¹è¿­ÀÇ ¸Þ´º°¡ ÀÏÄ¡ÇÏ¸é 0À» ¹ÝÈ¯. 
      		printf("À½·á: %s \n°¡°Ý: %s\n", menu_info[i].drink, menu_info[i].price);
         return i; //0ÀÌ µÇ¸é true°¡ µÇ¾î if¹®¿¡ µé¾î°¨. 
}
	   else{
  printf(" ÇØ´ç ¸Þ´º ¾ø½À´Ï´Ù \n");
  printf(" ¸Þ´º·Î µ¹¾Æ°¡°Ú½À´Ï´Ù \n");
  break;
 		}
	}
   return -1; //for¹® µ¹¾Æ°¡µµ Á¤º¸¸¦ ¸ø Ã£°Ô µÉ °æ¿ì 
}
...
int search_customer()
{
   ...
   {
      if(strcmp(name, customer_info[i].name)==0){  //°í°´ÀÇ À¯¹«¿¡ µû¶ó ¹ÝÈ¯ÇÏ±â À§ÇÑ ÇüÅÂ
      printf("°í°´ ÀÌ¸§: %s \n°í°´ ÀüÈ­¹øÈ£: %s\n", customer_info[i].name, customer_info[i].phone);	
    return i;
  }
         
         else{
  printf(" ÇØ´ç °í°´ÀÌ ¾ø½À´Ï´Ù \n");
  printf(" ¸Þ´º·Î µ¹¾Æ°¡°Ú½À´Ï´Ù \n");
  break;
 		}
   return -1;
